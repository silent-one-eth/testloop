...
export default {
  ...
  data: () => ({ loading: true, balance: null }),
  computed: {
    isEthereum() {
      return this.token.token == "ETH";
    },
    hasUnit() {
  ...
  methods: {
    async getBalance() {
      ...
      this.loading = true;
      if (this.isEthereum) {
        const rawBalance = await web3.eth.getBalance(this.account);
        this.balance = web3.utils.fromWei(rawBalance, this.unit);
      } else {
        const tokenABI = this.getContractABI();
        const tokenInst = new web3.eth.Contract(tokenABI, this.token.address);
        const rawBalance = await tokenInst.methods.balanceOf(this.account).call();
        this.balance = web3.utils.fromWei(rawBalance, this.unit);
      }
      this.loading = false;
    },
